...
public class AssertLog {
     ...
     * @param message  Message to log if assertion fails
     */
    public static <T> void assertSize(final T[] array, final int expected, final String message) {
        Assert.assertEquals(array.length, expected, message);
    }
    ...
    public static void assertFileExists(final Path path) {
        Assert.assertTrue(path.toFile().exists(), "Expected file " + path + " not found");
    }
